...
module "launch_templates" {
  source          = "git::https://github.com/njibrigthain100/Cognitech-terraform-iac-modules.git//terraform/modules/Launch_template?ref=v1.3.17"
...
module "auto_scaling_groups" {
  ...
  Autoscaling_group = merge(
    each.value,
    {
      launch_configuration = try(module.launch_templates[each.value.launch_templates.key].name, each.value.name)
    },
