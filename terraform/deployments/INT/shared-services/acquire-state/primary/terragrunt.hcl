...
locals {
  ...
  state_bucket    = local.region_context == "primary" ? "${local.account_name}-${local.region_prefix.primary}-${local.vpc_name}-config-bucket" : "${local.account_name}-${local.region_prefix.secondary}-${local.vpc_name}-config-bucket"
